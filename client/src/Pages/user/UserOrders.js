                ...
                <div className='space-y-4'>
                    {orders.map((order) => (
                        <div key={order._id} className='bg-white shadow-lg rounded-lg p-5 border border-gray-200'>
                            ...
                            <p className='text-lg font-medium mb-2 text-green-700'>Total Price: ${order.totalPrice}</p>
                            ...
                            <ul className='list-disc pl-5'>
                                {order.items.map((item, index) => (
                                    <li key={index} className='mb-2 text-gray-600'>
                                        ...
                                        <p className='mt-1 text-gray-800'>Total Price: ${item.price * item.quantity}</p>
                                    </li>
                                ))}
                            </ul>
                        </div>
